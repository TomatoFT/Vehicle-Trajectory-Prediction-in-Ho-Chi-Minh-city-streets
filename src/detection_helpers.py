...
class Detector:
    ...
    def detect(self, source, plot_bb: bool = True):
        ...
        pred = self.model(img, augment=False)[
            0
        ]  # We don not need any augment during inference time
        ...
        # Apply Classifier
        if self.classify:
            pred = apply_classifier(
                pred, self.modelc, img, im0
            )  # I thnk we need to add a new axis to im0

        # Post - Process detections
        det = pred[
            0
        ]  # detections per image but as we have  just 1 image, it is the 0th index
        if len(det):
        ...
        return (
            im0 if plot_bb else None
        )  # just in case there's no detection, return the original image. For tracking purpose plot_bb has to be False always
