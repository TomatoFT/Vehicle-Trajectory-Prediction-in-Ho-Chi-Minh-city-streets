...
class YOLOv7_DeepSORT:
    ...
    def track_video(self, video: str, output: str, skip_frames: int = 0, show_live: bool = False, 
                    count_objects: bool = False, verbose: int = 0, data_gathering: bool = True, data_file: str = 'vehicle_data.csv'):
        '''
        ...
        if data_gathering:
            with open(data_file, 'w+') as file_data:
                file_data.writelines('Track_ID, Class_name, X_min, X_max, Y_min, Y_max \n')
        out = None
        ...
        while True:  # while video is running
            ...
            bboxes, scores, classes, num_objects = self.get_info_from_yolo_dets(yolo_dets)
            # ---------------------------------------- DETECTION PART COMPLETED ---------------------------------------------------------------------

            names, count = self.handle_counting_objects(count_objects, frame, classes, num_objects)

            # ---------------------------------- DeepSORT tacker work starts here ------------------------------------------------------------
            # encode detections and feed to tracker. [No of BB / detections per frame, embed_size]
            detections, colors = self.get_detection_and_color(frame, bboxes, scores, names)
            ...
            for track in self.tracker.tracks:  # update new findings AKA tracks
                ...
                if str(track.track_id) not in hash_id.keys():
                    ...
                    print('Initial Completed for ID ', track.track_id)
                    print('Hash ID keys: ', hash_id.keys())
                    
                    
                # self.visualize_the_trajectory_prediction(hash_id, frame, class_name, track, bbox, color)
                ...
                self.write_the_information_to_data_file(data_gathering, data_file, class_name, track, bbox)
    ...
    def handle_counting_objects(self, count_objects, frame, classes, num_objects):
        ...
        if count_objects:
            cv2.putText(frame, "Tracking Object: {}".format(
                    count), (5, 35), cv2.FONT_HERSHEY_DUPLEX, 1.5, (0, 255, 0), 2)
    ...
    def write_the_information_to_data_file(self, data_gathering, data_file, class_name, track, bbox):
        if data_gathering:
    ...
    def visualize_tracking(self, frame, class_name, track, bbox, color):
        ...
        cv2.circle(frame, (abs(int(bbox[0] + bbox[2]/2 - bbox[0]/2)), abs(
                            int(bbox[1] + bbox[3]/2 - bbox[1]/2))), radius=5, color=color, thickness=5)
        cv2.putText(frame, class_name + " : " + str(track.track_id), (int(
                    bbox[0]), int(bbox[1]-11)), 0, 0.6, (255, 255, 255), 1, lineType=cv2.LINE_AA)

    def visualize_the_trajectory_prediction(self, hash_id, frame, class_name, track, bbox, color):
        predict_model = tf.keras.models.load_model('models/CNN_GRU.h5')
        if len(hash_id[f'{track.track_id}']) == 16+5:
            hash_id[f'{track.track_id}'].pop(5)
            hash_id[f'{track.track_id}'].pop(5)
            hash_id[f'{track.track_id}'].append(
                        [abs(int(bbox[0] + bbox[2]/2 - bbox[0]/2))]), hash_id[f'{track.track_id}'].append([abs(int(bbox[1] + bbox[3]/2 - bbox[1]/2))])
            sample = np.reshape(
                        hash_id[f'{track.track_id}'], (1, 21, 1))
            prediction = predict_model.predict(sample)
                    # x_next, y_next = prediction[0], prediction[1]
            print('Predict X and y: ', int(
                        prediction[0][0]), int(prediction[0][1]))
            cv2.circle(frame, (int(prediction[0][0]), int(
                        prediction[0][1])), radius=5, color=color, thickness=5)
            cv2.line(frame, (abs(int(bbox[0] + bbox[2]/2 - bbox[0]/2)), abs(int(bbox[1] + bbox[3]/2 - bbox[1]/2))), (int(
                        prediction[0][0]), int(prediction[0][1])), color=color, thickness=5)
