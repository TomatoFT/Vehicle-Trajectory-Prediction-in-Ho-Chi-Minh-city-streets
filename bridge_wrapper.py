...
class YOLOv7_DeepSORT:
    ...
    def track_video(self,video:str, output:str, skip_frames:int=0, show_live:bool=False, count_objects:bool=False, verbose:int = 0, data_gathering:bool=True):
        '''
        ...
        if data_gathering:
            os.remove('vehicle_data.csv')
            with open('vehicle_data.csv','w') as file_data:
                file_data.writelines('Track_ID, Class_name, X, Y \n')
        out = None
        ...
        while True: # while video is running
            ...
            if count_objects:
                cv2.putText(frame, "Tracking Object: {}".format(count), (5, 35), cv2.FONT_HERSHEY_DUPLEX, 1.5, (0, 255, 0), 2)
            ...
            for track in self.tracker.tracks:  # update new findings AKA tracks
                ...
                if len(hash_id[f'{track.track_id}'])==16+5:
                    ...
                    hash_id[f'{track.track_id}'].pop(5)
                    hash_id[f'{track.track_id}'].append([abs(int(bbox[0] + bbox[2]/2 - bbox[0]/2))]), hash_id[f'{track.track_id}'].append([abs(int(bbox[1] + bbox[3]/2 - bbox[1]/2))])
                    sample = np.reshape(hash_id[f'{track.track_id}'], (1,21,1))
                    prediction = predict_model.predict(sample)
                    # x_next, y_next = prediction[0], prediction[1]
                    print('Predict X and y: ',int(prediction[0][0]),int(prediction[0][1]))
                    cv2.circle(frame,(int(prediction[0][0]),int(prediction[0][1])),radius=5, color=color,thickness=5)
                    cv2.line(frame, (abs(int(bbox[0] + bbox[2]/2 - bbox[0]/2)),abs(int(bbox[1] + bbox[3]/2 - bbox[1]/2))), (int(prediction[0][0]),int(prediction[0][1])), color=color,thickness=5)
                    cv2.putText(frame, "Predicted of id " + str(track.track_id),(int(prediction[0][0]),int(prediction[0][1])+5),0, 0.6, (255,0,0),1, lineType=cv2.LINE_AA)
                ...
                cv2.rectangle(frame, (int(bbox[0]), int(bbox[1]-30)), (int(bbox[0])+(len(class_name)+len(str(track.track_id)))*17, int(bbox[1])), color, -1)
                cv2.circle(frame,(abs(int(bbox[0] + bbox[2]/2 - bbox[0]/2)),abs(int(bbox[1] + bbox[3]/2 - bbox[1]/2))),radius=5, color=color,thickness=5)
                ...
                if data_gathering:
                    with open('vehicle_data.csv','a+') as file_data:
                        file_data.writelines(f'{track.track_id},{class_name},{abs(int(bbox[0] + bbox[2]/2 - bbox[0]/2))},{abs(int(bbox[1] + bbox[3]/2 - bbox[1]/2))} \n')
